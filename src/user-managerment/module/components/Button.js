...
export const OrderRecieveButton = ({
  ...
  //let buttonText="";
  if (
    session === SessionEvent.MADEPAYMENT ||
    session === SessionEvent.SESSIONCREATED
  ) {
...
export const OrderTransferButton = ({
  type = "button",
  disabled,
  onClick,
  loading,
  ...
  //let buttonText="";
  if (session === SessionEvent.MADEPAYMENT) {
    text = "Confirming....";
  }
  if (session === SessionEvent.VERIFIEDPAYMENT || session === SessionEvent.ESCROWPUSHCOMPLETE) {
    text = "Order Verified";
  }
  if (session === SessionEvent.CANCELLED) {
